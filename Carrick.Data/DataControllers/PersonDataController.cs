namespace Carrick.Data.Controllers
{
    ...
    public class PersonDataController : GenericDataController<Person>
    {
        internal PersonDataController(Repository r) : base(r, r.DataModel.People)
        {
            try
            {
        ...
        public override Person[] GetActiveItems()
        {
            return base.GetActiveItems().Where(x => (x.IsDeleted == false)
                    && (x.DateLeftOrganisation == null)).ToArray<Person>();
        }

        public Person[] GetChildren(Person p)
        {
            return Repository.PersonPersonDataController.GetRelatedAPersons(p, 1);
        }

        public Person[] GetParents(Person p)
        {
